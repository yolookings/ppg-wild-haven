...
export class CollectionBookPanel extends Phaser.GameObjects.Container {
  ...
  private createAreaTabs(_panelWidth: number, panelHeight: number): void {
    ...
    areas.forEach((area, idx) => {
      ...
      const tabBg = this.scene.add.nineslice(0, 0, 'button', 0, tabWidth - 4, 30, 6, 6, 6, 6);
      ...
      const tabText = this.scene.add.text(0, 0, area.name.split(' ')[0], {
        fontFamily: 'Outfit, sans-serif',
        fontSize: '12px',
        fontStyle: 'bold',
        color: '#5c4832'
  ...
  public refresh(): void {
    ...
    creatures.forEach((creature, idx) => {
      ...
      // Card Background
      const cardBg = this.scene.add.nineslice(0, 0, 'button', 0, 120, 105, 8, 8, 8, 8);
      ...
      sprite.setScale(1.2); // Smaller card preview
